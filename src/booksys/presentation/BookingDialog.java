...
abstract class BookingDialog extends Dialog
{
  ...
  BookingDialog(Frame owner, String title, Booking booking)
  {
    ...
    Enumeration enum1 = BookingSystem.getTableNumbers().elements() ;
    while (enum1.hasMoreElements()) {
      tableNumber.add(((Integer) enum1.nextElement()).toString()) ;
    }
    if (booking != null) {
    ...
    choice = new Button("ChoiceMenu") ;
    choice.addActionListener(new ActionListener() {
    	@Override
	public void actionPerformed(ActionEvent e) {
		SelectMenuDialog s
	    = new SelectMenuDialog(owner,
			      "Select Menu") ;
